...
input="Competitions: competition_id int title varchar starts_on timestamp finishes_on timestamp created_on timestamp modified_on timestamp \n" +
...
var result = schema.trim().split('\n').map(function(line){
	...
	sql_insert = fmt.insert.format({table: table, cols: flds.join(', '), placeholders: flds.reduce(function(state, item){return state + ':' + item.replace(/`/g, '') + ', '}, '').replace(/, $/, '')});
