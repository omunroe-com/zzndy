 ...
 * @param {Number} selectionPart        percent of instances selected for breeding
 * @param {Number} mutationProbability  possibility of gene mutation
 */
Genetic = function( population, fitness, selectionPart, mutationProbability )
{
    this.population = population;
    this.fitness = fitness;
    this.nBreedSelect = Math.round(population.length * selectionPart / 100);
...
function crossover( a, b )
{
    ...
    // Mutate
    if ( Math.random() < this.mutationProbability / 100 )
    {
        ...
        var mutated = chars[n];
        c = c.slice(0, i) + mutated + c.slice(i + 1, c.length);
    }
