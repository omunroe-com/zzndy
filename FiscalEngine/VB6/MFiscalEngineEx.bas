Attribute VB_Name = "MFiscalEngineEx"
...
Public Const RunSwitches_RunType = 1              ' type of run to do
Public Const RunSwitches_RunTypeStd = 1               ' type of run is standard project
Public Const RunSwitches_RunTypeConsol = 2            ' type of run is post-tax consol
    
Public Const RunSwitches_InitAccum = 2            ' whether or not to initialize AC, CC, L1
Public Const RunSwitches_InitAccumNo = 0              ' no, do not initialize
Public Const RunSwitches_InitAccumYes = 1             ' yes, initialize
...
Const MY3_Use_AWIN = -998        ' This is the old WIN constant code
...
Const pRateCode_RTO = gc_nRtPrmRTO  ' This is the CURRENT value that indicates
...
Const pRateCode_UserBase = 100   ' This is the CURRENT value that is added to
...
'     (C0911)
'
Public Function FiscalCalculatorEx( _
    ...
    ByRef ProjectDates() As String, ByRef MiscellaneousItems() As Double, _
    ByRef LoansData() As Double, ByRef BaseAmountsExtraIn() As Double, ByRef CapexPARIn() As Double, _
    ByRef ModelVarsIn() As String, ByRef ModelVarsAmountsIn() As Double, ByRef ModelVarsToReturn() As String, _
    ByVal CountryFileName As String, ByRef CountryFileSensitivities() As String, _
    ...
    ByRef CalcIndicators() As Integer, ByRef OutputText As CReportText, _
    ByRef RunSwitches() As Long _
    ) As Boolean

    On Error GoTo FiscalCalculator_Error
    
    Const settings_discount_method = 1
    Const settings_field_type = 2
    Const settings_allocation_method = 3
    Const settings_abandonment_timing = 4
...
FiscalCalculator_Initialize:
        ...
        ReDim Inflate(1 To gc_nMAXLIFE, 1 To 2)
...
FiscalCalculator_InitializeStandard:        ' This is the target of a goto, coming from FiscalCalculator_Initialize
        ...
        Set l_oRunFileOut = g_oFileSystem.OpenForOutput(vbNullString)
        With l_oRunFileOut
            ...
            If StrComp(Right(sTemp, 4), ".cty", vbTextCompare) = 0 Then
        ...
        Set l_oRunFile = l_oRunFileOut      ' set the now-closed file reference
...
'<<TBD>> Proper size of economic indicators array??? 22 indicators?
        ReDim EconomicIndicators(1 To 6, 1 To 14)
...
FiscalCalculator_Input:
        ...
        ConvertCapexForecasts _
            zzzArray2DDoubleAsSingle(CapexAmounts(), mainexecCompatibilityMode), _
            zzzArray2DDoubleAsSingle(CapexWI(), mainexecCompatibilityMode), _
            zzzArray2DDoubleAsSingle(CapexRP(), mainexecCompatibilityMode), _
            zzzArray1DDoubleAsSingle(CapexTangible(), mainexecCompatibilityMode), _
            CalcSettings(settings_allocation_method), _
            YR, _
            my3(), _
            MY3T
...
'<<TBD>> Check for LG = 0? What to do if no volume forecasts
        ...
        ReDim A(1 To gc_nMAXLIFE, 1 To UBound(BaseAmounts, 1))
        ' was:
...
'<<TBD>> extract the abandonment inflation from Inflation()
        ...
        Set g_oRunFileIn = l_oRunFile.OpenForInput
...
FiscalCalculator_Output:
        ...
        Set l_oAmountsOut = New CDASPEAmountsSeq
        ...
        Set l_oMap = New CDGiantRptPageMapSeqB      ' amounts array
        g_oReport.WriteReport l_oAmountsOut, l_oMap
        OutputAmounts = l_oAmountsOut.AllValues
        
        ' Get interests array from report object
        l_oAmountsOut.Initialize i, 2
        Set l_oMap = New CDGiantRptPageMapSeqC      ' interests array
        g_oReport.WriteReport l_oAmountsOut, l_oMap
        OutputInterests = l_oAmountsOut.AllValues
        
        Set l_oNamesOut = New CDASPENamesSeq
        
        ' Get names array from report object
        l_oNamesOut.Initialize i, 2
        Set l_oNamesMap = New CDGiantRptPageMapSeqD      ' names array
        ' Make Report Text object available to the names map
        ' so it can assign it to the report page formats
        With l_oNamesMap
            Set .ReportText = g_oReport
            Set .VariableTitles = g_oReport
        End With
        g_oReport.WriteReport l_oNamesOut, l_oNamesMap
        OutputNames = l_oNamesOut.AllNames
...
'''        EconomicLimitApplied = IIf(zzzFiscalDefinitionHasLMTKeyword, 1, 0)
        ...
        Set l_oSummary = New CDArray1DDblA          ' written to array
        l_oSummary.Initialize 1, 16                 ' allocate receiving array, need 16 items (was 14, but added 3d party, NOC)
        Set l_oMap = New CCMGiantRunSummarySeqB     ' provides format to output the 16 items
        g_oReport.WriteReport l_oSummary, l_oMap
        OtherIndicators = l_oSummary.AllValues           ' assign the output array
...
'
Private Sub zzzCheckInputForecastsLife(ByRef InputForecasts() As Double, ByRef ProjectLife As Integer, ByRef ProductionLife As Integer)
...
'
Private Sub zzzUpdateCapexWI(ByRef Forecasts() As Single, ByRef Capex() As Single, ByVal CapexCount As Integer)

   Dim rpx As Integer         ' period of capital expenditure
...
'
Private Function zzzFiscalDefinitionHasLMTKeyword()
    ...
    For i = 1 To UBound(TD$, 1)
        If StrComp(Left(TD$(i, 1), 3), "LMT", vbTextCompare) = 0 Then
            bResult = True
            Exit For
        End If
    Next i
...
'
Private Function zzzArray1DDoubleAsSingle _
    ( _
    ByRef InputArray() As Double, _
    ByVal mainexecCompatibilityMode As Boolean _
    ) As Single()
    ...
    ReDim sOutput(LBound(InputArray, 1) To UBound(InputArray, 1))
    ...
    zzzArray1DDoubleAsSingle = sOutput()
...
' for comparing with Mainexec.
'
Private Function zzzArray2DDoubleAsSingle _
    ( _
    ByRef InputArray() As Double, _
    ByVal mainexecCompatibilityMode As Boolean _
    ) As Single()
    ...
    ' Copy the double precision input amounts to the single precision output array
    ReDim sOutput(LBound(InputArray, 1) To UBound(InputArray, 1), LBound(InputArray, 2) To UBound(InputArray, 2))
    ...
    zzzArray2DDoubleAsSingle = sOutput()
...
' false otherwise.
'
Private Function IsAllocatedArrayStr(test() As String)
