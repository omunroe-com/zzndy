//(function() {
    ...
    /**
     * Yield a range of numbers from zero to n-1
     * @param {Number} n  length of range to return
     */
    function range(n) {
        var ar = []
    ...
    /**
     * Repeat this string given number of times
     * @param {Number} n  times to repeat this string
     */
    S.x = function(n) {
    ...
    N.times = function(fn, self) {
        ...
        if (typeof fn == 'function')
            for (; i < this; ++i) res.push(fn.call(this, i, self))
        else if (typeof fn == 'object' || fn instanceof Array)
            for (; i < this; ++i) res.push(fn.clone())
        else
            for (; i < this; ++i) res.push(fn)
     ...
     * Watch the case
     *
     * @param {Object} fmtObj  template object with values to incert
     */
    S.format =
    S.fmt = function(fmtObj) {
        ...
        function simpleFormat(a) {
            ...
            for (var k in a) {
                ...
                if (typeof k == 'string')
                    rx = new RegExp('<' + k.charAt(0).toUpperCase() + k.substr(1) + '>', 'g')
                else rx = new RegExp('<' + k + '>', 'g')
                r = r.replace(rx, a[k])
            }
            return r
        }
    ...
    A.clone = function(n) {
        var rval = [].concat(this);
        if (n && n > 1) {
            var l = this.length;
            for (var i = 0; i < l; ++i) {
                rval[i] = this[i].clone(n - 1);
            }
        }
        return rval;
    }
