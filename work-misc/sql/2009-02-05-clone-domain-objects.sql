...
BEGIN
	...
	BEGIN TRY
		...
		SELECT * INTO #GLOBAL_ASSUMPTIONS_GLOBALS
			FROM GLOBAL_ASSUMPTIONS WHERE 42=10;

		SELECT * INTO #GAS_PRICE_GLOBALS
			FROM GAS_PRICE WHERE 42=10;

		SELECT * INTO #INFLATION_GLOBALS
			FROM INFLATION WHERE 42=10;

		SELECT * INTO #LIQUID_PRICE_GLOBALS
			FROM LIQUID_PRICE WHERE 42=10;

		SELECT * INTO #GAS_PRICE_DATA_GLOBALS
			FROM GAS_PRICE_DATA WHERE 42=10;

		SELECT * INTO #INFLATION_DATA_GLOBALS
			FROM INFLATION_DATA WHERE 42=10;

		SELECT * INTO #LIQUID_PRICE_DATA_GLOBALS
		...
		INSERT INTO #LIQUID_PRICE_GLOBALS
		...
		INSERT INTO #GAS_PRICE_DATA_GLOBALS
			SELECT * FROM GAS_PRICE_DATA
			WHERE GAS_PRICE_ID IN (SELECT GAS_PRICE_ID FROM GAS_PRICE WHERE GLOBAL_ASSUMPTIONS_ID = @GLOBAL_ASSUMPTIONS_ID);

		INSERT INTO #INFLATION_DATA_GLOBALS
			SELECT * FROM INFLATION_DATA
			WHERE INFLATION_ID IN (SELECT INFLATION_ID FROM INFLATION WHERE GLOBAL_ASSUMPTIONS_ID = @GLOBAL_ASSUMPTIONS_ID);

		INSERT INTO #LIQUID_PRICE_DATA_GLOBALS
			SELECT * FROM LIQUID_PRICE_DATA
			WHERE LIQUID_PRICE_ID IN (SELECT LIQUID_PRICE_ID FROM LIQUID_PRICE WHERE GLOBAL_ASSUMPTIONS_ID = @GLOBAL_ASSUMPTIONS_ID);

		DECLARE @NAME VARCHAR(50)
		SELECT @NAME = 'Copy of ' + GLOBALS_NAME FROM #GLOBAL_ASSUMPTIONS_GLOBALS WHERE GLOBAL_ASSUMPTIONS_ID = @GLOBAL_ASSUMPTIONS_ID;
		...
		BEGIN
			SELECT @NAME = 'Copy (' + CONVERT(VARCHAR(2), @N) + ') of ' + GLOBALS_NAME
				FROM #GLOBAL_ASSUMPTIONS_GLOBALS WHERE GLOBAL_ASSUMPTIONS_ID = @GLOBAL_ASSUMPTIONS_ID;
		
			SET @N = @N + 1;
		END

		UPDATE #GLOBAL_ASSUMPTIONS_GLOBALS SET GLOBALS_NAME = @NAME WHERE GLOBAL_ASSUMPTIONS_ID = @GLOBAL_ASSUMPTIONS_ID;
		...
		UPDATE #GLOBAL_ASSUMPTIONS_GLOBALS SET GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID WHERE GLOBAL_ASSUMPTIONS_ID = @GLOBAL_ASSUMPTIONS_ID;

		DECLARE @NEW_GAS_PRICE_ID DECIMAL(12, 0);

		DECLARE CURS CURSOR
		FOR SELECT DISTINCT GAS_PRICE_ID
			FROM #GAS_PRICE_GLOBALS
		...
		BEGIN
			...
			UPDATE #GAS_PRICE_GLOBALS
				SET GAS_PRICE_ID = @NEW_GAS_PRICE_ID
				WHERE GAS_PRICE_ID = @GAS_PRICE_ID;

			UPDATE #GAS_PRICE_GLOBALS SET GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID WHERE GAS_PRICE_ID = @GAS_PRICE_ID;
			UPDATE #GAS_PRICE_DATA_GLOBALS SET GAS_PRICE_ID = @NEW_GAS_PRICE_ID WHERE GAS_PRICE_ID = @GAS_PRICE_ID;
		...
		BEGIN
			...
			UPDATE #INFLATION_GLOBALS
				SET INFLATION_ID = @NEW_INFLATION_ID
				WHERE INFLATION_ID = @INFLATION_ID;

			UPDATE #INFLATION_GLOBALS SET GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID WHERE INFLATION_ID = @INFLATION_ID;
			UPDATE #INFLATION_DATA_GLOBALS SET INFLATION_ID = @NEW_INFLATION_ID WHERE INFLATION_ID = @INFLATION_ID;
		...
		BEGIN
			...
			UPDATE #LIQUID_PRICE_GLOBALS
				SET LIQUID_PRICE_ID = @NEW_LIQUID_PRICE_ID
				WHERE LIQUID_PRICE_ID = @LIQUID_PRICE_ID;

			UPDATE #LIQUID_PRICE_GLOBALS SET GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID WHERE LIQUID_PRICE_ID = @LIQUID_PRICE_ID;
			UPDATE #LIQUID_PRICE_DATA_GLOBALS SET LIQUID_PRICE_ID = @NEW_LIQUID_PRICE_ID WHERE LIQUID_PRICE_ID = @LIQUID_PRICE_ID;
		...
		INSERT INTO GLOBAL_ASSUMPTIONS
			SELECT * FROM #GLOBAL_ASSUMPTIONS_GLOBALS
			WHERE GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID;

		INSERT INTO GAS_PRICE
			SELECT * FROM #GAS_PRICE_GLOBALS
			WHERE GAS_PRICE_ID IN (SELECT GAS_PRICE_ID FROM GAS_PRICE_GLOBALS WHERE GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID);

		INSERT INTO INFLATION
			SELECT * FROM #INFLATION_GLOBALS
			WHERE INFLATION_ID IN (SELECT INFLATION_ID FROM INFLATION_GLOBALS WHERE GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID);

		INSERT INTO LIQUID_PRICE
			SELECT * FROM #LIQUID_PRICE_GLOBALS
			WHERE LIQUID_PRICE_ID IN (SELECT LIQUID_PRICE_ID FROM LIQUID_PRICE_GLOBALS WHERE GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID);

		INSERT INTO GAS_PRICE_DATA
			SELECT * FROM #GAS_PRICE_DATA_GLOBALS
			WHERE GAS_PRICE_ID IN (SELECT GAS_PRICE_ID FROM GAS_PRICE_GLOBALS WHERE GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID);

		INSERT INTO INFLATION_DATA
			SELECT * FROM #INFLATION_DATA_GLOBALS
			WHERE INFLATION_ID IN (SELECT INFLATION_ID FROM INFLATION_GLOBALS WHERE GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID);

		INSERT INTO LIQUID_PRICE_DATA
			SELECT * FROM #LIQUID_PRICE_DATA_GLOBALS
			WHERE LIQUID_PRICE_ID IN (SELECT LIQUID_PRICE_ID FROM LIQUID_PRICE_GLOBALS WHERE GLOBAL_ASSUMPTIONS_ID = @NEW_GLOBAL_ASSUMPTIONS_ID);
		...
		DROP TABLE #GLOBAL_ASSUMPTIONS_GLOBALS;

		DROP TABLE #GAS_PRICE_GLOBALS;

		DROP TABLE #INFLATION_GLOBALS;

		DROP TABLE #LIQUID_PRICE_GLOBALS;

		DROP TABLE #GAS_PRICE_DATA_GLOBALS;

		DROP TABLE #INFLATION_DATA_GLOBALS;

		DROP TABLE #LIQUID_PRICE_DATA_GLOBALS;
...
BEGIN
	...
	BEGIN TRY
		...
		UPDATE COMPANY_ADDITIONAL SET CREATED_BY='USER' WHERE PU_ID = @PU_ID;
...
BEGIN
	...
	BEGIN TRY
		...
		UPDATE TAX_SYSTEM SET CREATED_BY='USER' WHERE TAX_SYSTEM_ID = @TAX_SYSTEM_ID;
